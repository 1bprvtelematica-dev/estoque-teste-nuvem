# =============================================================================
# DOCUMENTACAO RESUMIDA - CONTROLE DE ESTOQUE 1 BPRv
# =============================================================================
# Arquivo principal: appv7.1.py
# Plataforma: Streamlit + SQLite
# Banco: estoque.db, localizado na mesma pasta do aplicativo.
#
# Objetivo do sistema:
# - Controlar cadastro, entrada, saida, rastreabilidade e recibos de materiais.
# - Registrar historico completo de movimentacoes por fornecedor, cliente e recibo.
# - Oferecer dashboard, relatorios, auditoria, backup automatico e controle de usuarios.
#
# Principais areas deste arquivo:
# 1. CSS GLOBAL / ESTILIZACAO
#    Define tema visual, fundo, login, sidebar, botoes, inputs, tabelas e recibos.
#    A estilização usa seletores reais do Streamlit/BaseWeb e CSS injetado via st.markdown.
#
# 2. BANCO DE DADOS
#    Cria/atualiza tabelas: usuarios, materiais, fornecedores, clientes, orgaos,
#    historico, seq_recibo, logs_sistema, solicitacoes_usuarios e solicitacoes_senha.
#    Tambem cria indices unicos para patrimonio, serie, IMEI, numero de linha e chip.
#
# 3. SEGURANCA E LOGIN
#    Usa bcrypt para senha, bloqueio temporario por tentativas invalidas,
#    timeout por inatividade, solicitacao de novo usuario e redefinicao de senha
#    aprovada pelo administrador.
#
# 4. MOVIMENTACAO
#    Registrar Entrada: adiciona materiais unitarios ou genericos ao estoque e historico.
#    Registrar Saida: baixa estoque com transacao, gera recibo numerado e limpa os inputs.
#    Editor de recibo: altera observacoes, remove item devolvendo ao estoque e adiciona item.
#
# 5. CADASTROS E GERENCIAMENTO
#    Cadastro/edicao de materiais, clientes, fornecedores e orgaos.
#    Bloqueia exclusao de material com historico para preservar rastreabilidade.
#
# 6. RELATORIOS E AUDITORIA
#    Dashboard operacional, busca, reemissao de recibo, relatorios Excel/PDF,
#    backup do banco, logs e limpeza controlada por administrador.
#
# Organizacao futura recomendada:
# - Este arquivo esta funcional, mas grande. Para evolucao/manutencao, o ideal e modularizar:
#   config.py, db.py, auth.py, services/estoque.py, services/recibos.py,
#   ui/login.py, ui/cadastros.py, ui/movimentacoes.py, ui/relatorios.py e styles.py.
# - A modularizacao deve ser feita em etapa separada, com testes apos cada extracao,
#   para evitar quebrar regras sensiveis de estoque, recibo e historico.
#
# Documentacao completa: DOCUMENTACAO_APPV7_1.md
# =============================================================================
...
st.set_page_config(
    page_title="Controle de Estoque 1º BPRv",
    layout="wide",
...
# ═══════════════════════════════════════════════════════════════════════════════
def get_conn():
    conn = sqlite3.connect(str(Path(__file__).resolve().parent / "estoque.db"), check_same_thread=False)
    conn.execute("PRAGMA journal_mode=WAL")
    return conn

APP_DIR = Path(__file__).resolve().parent
DB_PATH = APP_DIR / "estoque.db"
BACKUP_CONFIG_PATH = APP_DIR / "backup_config.json"
...
def carregar_config_backup():
    cfg = BACKUP_CONFIG_DEFAULT.copy()
...
def salvar_config_backup(cfg):
    cfg_limpa = BACKUP_CONFIG_DEFAULT.copy()
    cfg_limpa.update(cfg)
    with open(BACKUP_CONFIG_PATH, "w", encoding="utf-8") as f:
        json.dump(cfg_limpa, f, ensure_ascii=False, indent=2)

def intervalo_backup_dias(cfg):
    freq = cfg.get("frequencia", "Diário")
    if freq == "Semanal":
        return 7
    if freq == "Mensal":
        return 30
    if freq == "A cada X dias":
        return max(1, int(cfg.get("intervalo_dias") or 1))
    return 1

def backup_esta_vencido(cfg):
    if not cfg.get("ativo") or not (cfg.get("destino") or "").strip():
        return False
    ultimo = (cfg.get("ultimo_backup") or "").strip()
    if not ultimo:
        return True
    try:
        ...
        return True
    return datetime.now() - dt_ultimo >= timedelta(days=intervalo_backup_dias(cfg))

def arquivos_configuracao_para_backup():
    nomes = [
        "backup_config.json", ".streamlit/config.toml",
        "brasao_recibo.jpg", "brasao_recibo.png", "polrv.jpg", "polrv.png",
        "fundo3.jpg", "fundo.png", "fundo.jpg", "fundo.jpeg",
        "fundo4.jpg", "fundo 4.jpg", "fundo2.jpg", "fundo7.jpg",
...
def limpar_backups_antigos(destino, manter_ultimos):
    manter_ultimos = max(1, int(manter_ultimos or 1))
    backups = sorted(
        destino.glob("backup_estoque_*.zip"),
...
def criar_backup_automatico(cfg, usuario="sistema"):
    destino = Path((cfg.get("destino") or "").strip()).expanduser()
    if not destino:
        return False, "Informe uma pasta de destino."
    destino.mkdir(parents=True, exist_ok=True)
    if not destino.is_dir():
        return False, "Destino informado não é uma pasta."
    if not DB_PATH.exists():
        return False, "Banco de dados não encontrado."

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    temp_db = destino / f"estoque_snapshot_{stamp}.db"
    zip_path = destino / f"backup_estoque_{stamp}.zip"
    ...
    try:
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
            z.write(temp_db, "estoque.db")
            for arq in arquivos_configuracao_para_backup():
                z.write(arq, f"configuracoes/{arq.relative_to(APP_DIR).as_posix()}")
    finally:
        try:
            temp_db.unlink()
        except OSError:
    ...
    cfg["ultimo_backup"] = datetime.now().isoformat(timespec="seconds")
    salvar_config_backup(cfg)
    limpar_backups_antigos(destino, cfg.get("manter_ultimos", 30))
    registrar_log(usuario, f"Backup automatico gerado: {zip_path}")
    return True, str(zip_path)

def verificar_backup_automatico(usuario="sistema"):
    cfg = carregar_config_backup()
    if not backup_esta_vencido(cfg):
        return
    try:
        ok, msg = criar_backup_automatico(cfg, usuario)
        st.session_state["backup_auto_status"] = ("ok" if ok else "erro", msg)
    except Exception as exc:
        st.session_state["backup_auto_status"] = ("erro", str(exc))

def inicializar_banco():
    conn = get_conn(); c = conn.cursor()
    c.executescript("""
        CREATE TABLE IF NOT EXISTS usuarios(id INTEGER PRIMARY KEY AUTOINCREMENT,usuario TEXT UNIQUE NOT NULL,senha TEXT NOT NULL,perfil TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS materiais(id INTEGER PRIMARY KEY AUTOINCREMENT,nome TEXT NOT NULL,quantidade INTEGER NOT NULL DEFAULT 0,minimo INTEGER NOT NULL DEFAULT 0,patrimonio TEXT,num_serie TEXT,marca TEXT,modelo TEXT,observacoes TEXT,tipo_transceptor TEXT,prefixo_viatura TEXT,imei TEXT,lcm TEXT,orgao TEXT,entrada_registrada INTEGER NOT NULL DEFAULT 0,numero_linha TEXT,imei_chip TEXT);
        CREATE TABLE IF NOT EXISTS fornecedores(id INTEGER PRIMARY KEY AUTOINCREMENT,nome TEXT UNIQUE NOT NULL,telefone TEXT,cnpj TEXT,endereco TEXT,ramo TEXT,email TEXT);
        CREATE TABLE IF NOT EXISTS clientes(id INTEGER PRIMARY KEY AUTOINCREMENT,nome TEXT UNIQUE NOT NULL,doc_fiscal TEXT,campo_re TEXT,telefone TEXT,email TEXT);
        CREATE TABLE IF NOT EXISTS orgaos(id INTEGER PRIMARY KEY AUTOINCREMENT,nome TEXT UNIQUE NOT NULL);
        CREATE TABLE IF NOT EXISTS historico(id INTEGER PRIMARY KEY AUTOINCREMENT,tipo TEXT,material TEXT,quantidade INTEGER,entidade TEXT,identificacao TEXT,marca TEXT,modelo TEXT,observacoes TEXT,data_hora TEXT,num_recibo TEXT,seq_recibo INTEGER,material_id INTEGER);
        CREATE TABLE IF NOT EXISTS seq_recibo(id INTEGER PRIMARY KEY CHECK(id=1),valor INTEGER NOT NULL DEFAULT 0);
        CREATE TABLE IF NOT EXISTS logs_sistema(id INTEGER PRIMARY KEY AUTOINCREMENT,usuario TEXT,acao TEXT,data TEXT);
        CREATE TABLE IF NOT EXISTS solicitacoes_usuarios(id INTEGER PRIMARY KEY AUTOINCREMENT,usuario TEXT UNIQUE NOT NULL,senha TEXT NOT NULL,data TEXT NOT NULL);
        CREATE TABLE IF NOT EXISTS solicitacoes_senha(id INTEGER PRIMARY KEY AUTOINCREMENT,usuario TEXT UNIQUE NOT NULL,senha TEXT NOT NULL,data TEXT NOT NULL);
    """)
    # Migração dos nomes antigos, preservando todos os valores já cadastrados.
    colunas_materiais = {row[1] for row in c.execute("PRAGMA table_info(materiais)")}
    if "localizacao" in colunas_materiais and "tipo_transceptor" not in colunas_materiais:
        c.execute("ALTER TABLE materiais RENAME COLUMN localizacao TO tipo_transceptor")
        colunas_materiais.remove("localizacao")
        colunas_materiais.add("tipo_transceptor")
    if "cod_barras" in colunas_materiais and "prefixo_viatura" not in colunas_materiais:
        c.execute("ALTER TABLE materiais RENAME COLUMN cod_barras TO prefixo_viatura")
    for tbl, col, tp in [
        ("historico","marca","TEXT"),("historico","modelo","TEXT"),
        ("historico","observacoes","TEXT"),("historico","num_recibo","TEXT"),
        ("historico","seq_recibo","INTEGER"),
        ("historico","material_id","INTEGER"),
        ("materiais","tipo_transceptor","TEXT"),("materiais","prefixo_viatura","TEXT"),
        ("materiais","imei","TEXT"),("materiais","lcm","TEXT"),("materiais","orgao","TEXT"),
        ("materiais","entrada_registrada","INTEGER NOT NULL DEFAULT 0"),
        ("materiais","numero_linha","TEXT"),("materiais","imei_chip","TEXT"),
        ("fornecedores","cnpj","TEXT"),("fornecedores","endereco","TEXT"),("fornecedores","ramo","TEXT"),
        ("fornecedores","email","TEXT"),
    ]:
        try: c.execute(f"ALTER TABLE {tbl} ADD COLUMN {col} {tp}")
        except: pass
    c.execute("""
        INSERT OR IGNORE INTO orgaos(nome)
        SELECT DISTINCT TRIM(UPPER(orgao))
          FROM materiais
         WHERE TRIM(COALESCE(orgao,''))<>''
    """)
    c.execute("""
        UPDATE materiais
           SET entrada_registrada=1
         WHERE COALESCE(entrada_registrada,0)=0
           AND (
                quantidade<>0
                OR TRIM(COALESCE(patrimonio,''))<>''
                OR TRIM(COALESCE(num_serie,''))<>''
                OR TRIM(COALESCE(tipo_transceptor,''))<>''
                OR TRIM(COALESCE(prefixo_viatura,''))<>''
                OR TRIM(COALESCE(imei,''))<>''
                OR TRIM(COALESCE(lcm,''))<>''
                OR TRIM(COALESCE(orgao,''))<>''
                OR TRIM(COALESCE(numero_linha,''))<>''
                OR TRIM(COALESCE(imei_chip,''))<>''
        ...
        UPDATE materiais
         ...
         WHERE COALESCE(entrada_registrada,0)=0
           AND EXISTS (
                SELECT 1
                  FROM historico h
                 WHERE h.tipo='ENTRADA (FORNECEDOR)'
                   AND UPPER(h.material)=UPPER(materiais.nome)
           )
           AND NOT EXISTS (
                SELECT 1
                  FROM materiais m2
                 WHERE m2.id<>materiais.id
                   AND UPPER(m2.nome)=UPPER(materiais.nome)
                   AND COALESCE(m2.entrada_registrada,0)=1
            )
    """)
    grupos_genericos = c.execute("""
        SELECT UPPER(TRIM(COALESCE(nome,''))) nome_key,
               UPPER(TRIM(COALESCE(marca,''))) marca_key,
               UPPER(TRIM(COALESCE(modelo,''))) modelo_key
          FROM materiais
         WHERE TRIM(COALESCE(patrimonio,''))=''
           AND TRIM(COALESCE(num_serie,''))=''
           AND TRIM(COALESCE(imei,''))=''
           AND TRIM(COALESCE(numero_linha,''))=''
           AND TRIM(COALESCE(imei_chip,''))=''
         GROUP BY nome_key, marca_key, modelo_key
        HAVING COUNT(*) > 1
    """).fetchall()
    for nome_key, marca_key, modelo_key in grupos_genericos:
        rows = c.execute("""
            SELECT id,nome,quantidade,minimo,marca,modelo,observacoes,tipo_transceptor,prefixo_viatura,lcm,orgao,entrada_registrada
              FROM materiais
        ...
        """, (nome_key, marca_key, modelo_key)).fetchall()
        if len(rows) < 2:
            continue
        keeper = rows[0]
        ids_antigos = [r[0] for r in rows[1:]]
        qtd_total = sum(int(r[2] or 0) for r in rows)
        minimo = max(int(r[3] or 0) for r in rows)
        entrada_reg = 1 if any(int(r[11] or 0) for r in rows) else 0

        def primeiro_texto(idx):
            for r in rows:
                valor = (r[idx] or "").strip()
                if valor:
                    return valor
            return ""
        ...
        c.execute(
            """
            UPDATE materiais
               SET quantidade=?, minimo=?, observacoes=?, tipo_transceptor=?, prefixo_viatura=?,
                   lcm=?, orgao=?, entrada_registrada=?
             WHERE id=?
            """,
            (
                qtd_total, minimo, primeiro_texto(6), primeiro_texto(7),
                primeiro_texto(8), primeiro_texto(9), primeiro_texto(10),
                entrada_reg, keeper[0],
            ),
        )
        for id_antigo in ids_antigos:
            c.execute("UPDATE historico SET material_id=? WHERE material_id=?", (keeper[0], id_antigo))
            c.execute("DELETE FROM materiais WHERE id=?", (id_antigo,))
    if c.execute("SELECT COUNT(*) FROM seq_recibo").fetchone()[0] == 0:
        c.execute("INSERT INTO seq_recibo(id,valor) VALUES(1,?)", (RECIBO_SEQ_BASE,))
    else:
        c.execute("UPDATE seq_recibo SET valor=? WHERE id=1 AND valor<?", (RECIBO_SEQ_BASE, RECIBO_SEQ_BASE))
    for idx, col in [
        ("idx_materiais_patrimonio_unico", "patrimonio"),
        ("idx_materiais_serie_unica", "num_serie"),
        ("idx_materiais_imei_unico", "imei"),
        ("idx_materiais_numero_linha_unico", "numero_linha"),
        ("idx_materiais_imei_chip_unico", "imei_chip"),
    ]:
        try:
            c.execute(
                f"CREATE UNIQUE INDEX IF NOT EXISTS {idx} "
                f"ON materiais(UPPER({col})) "
    ...
    if c.execute("SELECT COUNT(*) FROM usuarios").fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO usuarios(usuario,senha,perfil) VALUES(?,?,?)", ("admin", h, "Administrador"))
...
def salvar_brasao_recibo(uploaded_file):
    ...
    if ext not in (".png", ".jpg", ".jpeg", ".webp"):
        ext = ".png"
    for nome in ("brasao_recibo.png", "brasao_recibo.jpg", "brasao_recibo.jpeg", "brasao_recibo.webp"):
        antigo = os.path.join(base_dir, nome)
        if os.path.exists(antigo):
            os.remove(antigo)
    destino = os.path.join(base_dir, f"brasao_recibo{ext}")
    with open(destino, "wb") as f:
        f.write(uploaded_file.getvalue())
    return os.path.basename(destino)

def brasao_recibo_data_url():
    if st.session_state.get("brasao_recibo"):
        return st.session_state["brasao_recibo"]
    for nome in ("brasao_recibo.png", "brasao_recibo.jpg", "brasao_recibo.jpeg", "brasao_recibo.webp"):
...
def saldo_agrupado_materiais(conn):
    df = pd.read_sql_query("""
        SELECT
            COALESCE(NULLIF(TRIM(nome),''),'SEM NOME') AS Material,
            COALESCE(TRIM(marca),'') AS Marca,
            COALESCE(TRIM(modelo),'') AS Modelo,
...
verificar_backup_automatico(st.session_state.get("usuario") or "sistema")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.info("O backup será gerado quando o sistema estiver em uso e a frequência configurada estiver vencida.")

    cfg_backup = carregar_config_backup()
    frequencias = ["Diário", "Semanal", "Mensal", "A cada X dias"]
    freq_atual = cfg_backup.get("frequencia", "Diário")
    if freq_atual not in frequencias:
        freq_atual = "Diário"

    status_backup = st.session_state.get("backup_auto_status")
    if status_backup:
        tipo_status, msg_status = status_backup
        if tipo_status == "ok":
            st.success(f"Último backup automático: {msg_status}")
        else:
            st.error(f"Falha no backup automático: {msg_status}")

    with st.form("form_backup_auto"):
        ativo = st.checkbox("Ativar backup automático", value=bool(cfg_backup.get("ativo")), key="backup_ativo")
        destino = st.text_input(
            "Pasta externa de destino",
            value=cfg_backup.get("destino", ""),
            placeholder=r"Ex: D:\BackupsEstoque ou \\SERVIDOR\Backups\Estoque",
            key="backup_destino",
        )
        c_freq, c_intervalo, c_manter = st.columns(3)
        frequencia = c_freq.selectbox(
            "Frequência",
            frequencias,
            index=frequencias.index(freq_atual),
            key="backup_frequencia",
        )
        intervalo_dias = c_intervalo.number_input(
            "Intervalo em dias",
            min_value=1,
            max_value=365,
            value=int(cfg_backup.get("intervalo_dias") or 1),
            step=1,
            disabled=frequencia != "A cada X dias",
            key="backup_intervalo",
        )
        manter_ultimos = c_manter.number_input(
            "Manter últimos backups",
            min_value=1,
            max_value=365,
            value=int(cfg_backup.get("manter_ultimos") or 30),
            step=1,
            key="backup_manter",
        )
        salvar_backup = st.form_submit_button("💾 Salvar configuração", use_container_width=True)
    ...
    if salvar_backup:
        novo_cfg = {
            ...
            "manter_ultimos": int(manter_ultimos),
            "ultimo_backup": cfg_backup.get("ultimo_backup", ""),
        }
        if ativo and not novo_cfg["destino"]:
            st.error("Informe a pasta externa antes de ativar o backup automático.")
        else:
            try:
                if novo_cfg["destino"]:
                    Path(novo_cfg["destino"]).expanduser().mkdir(parents=True, exist_ok=True)
                salvar_config_backup(novo_cfg)
                registrar_log(st.session_state["usuario"], "Alterou configuracao de backup automatico")
                st.success("Configuração de backup salva.")
                time.sleep(1); st.rerun()
            except Exception as exc:
                st.error(f"Não foi possível salvar/testar a pasta de destino: {exc}")

    st.markdown("---")
    st.subheader("Executar agora")
    c_info1, c_info2 = st.columns(2)
    ultimo_backup = cfg_backup.get("ultimo_backup") or "Nunca"
    if ultimo_backup != "Nunca":
        try:
            ultimo_backup = datetime.fromisoformat(ultimo_backup).strftime("%d/%m/%Y %H:%M:%S")
        except ValueError:
            pass
    c_info1.metric("Último backup", ultimo_backup)
    c_info2.metric("Próxima regra", cfg_backup.get("frequencia", "Diário"))

    if st.button("📦 Criar backup agora", type="primary", use_container_width=True, key="btn_backup_agora"):
        cfg_exec = carregar_config_backup()
        if not (cfg_exec.get("destino") or "").strip():
            st.error("Informe e salve uma pasta externa antes de criar o backup.")
        else:
            try:
                ok, msg = criar_backup_automatico(cfg_exec, st.session_state["usuario"])
                if ok:
                    st.success(f"Backup criado: {msg}")
                else:
                    st.error(msg)
            except Exception as exc:
    ...
    st.markdown("---")
    st.subheader("O que será salvo")
    st.write("O arquivo `.zip` inclui uma cópia consistente do banco `estoque.db` e arquivos de configuração/imagens do sistema quando existirem.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    conn = get_conn()
    df = pd.read_sql_query("SELECT id,usuario,acao,data FROM logs_sistema ORDER BY id DESC", conn)
    conn.close()
    tabela_visivel(df)
    if not df.empty:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tp:
        ...
        df_p = pd.read_sql_query(q, conn, params=p); conn.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if ok:
        if not nome.strip(): st.error("Nome é obrigatório.")
            ...
            except sqlite3.IntegrityError: st.error("Cliente já cadastrado.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if ok:
        if not nome.strip(): st.error("Nome é obrigatório.")
            ...
            except sqlite3.IntegrityError: st.error("Fornecedor já cadastrado.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if br:
        recibos_q = conn.execute("""
            ...
            GROUP BY num_recibo ORDER BY seq_recibo DESC
        """, (f"%{br}%",) * 5).fetchall()
    else:
        recibos_q = conn.execute("""
            SELECT DISTINCT num_recibo,seq_recibo,entidade,MAX(data_hora) data_hora
            FROM historico WHERE tipo='SAÍDA (CLIENTE)' AND num_recibo IS NOT NULL
            GROUP BY num_recibo ORDER BY seq_recibo DESC LIMIT 30
        """).fetchall()
    conn.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_ht = pd.read_sql_query("SELECT data_hora,tipo FROM historico", conn)
    if not df_ht.empty:
    ...
    with col_c:
        ...
        df_top = pd.read_sql_query("SELECT material 'Material',SUM(quantidade) 'Total',COUNT(*) 'Operações' FROM historico GROUP BY material ORDER BY 2 DESC LIMIT 10", conn)
        df_top.columns = ["Material", "Total", "Operacoes"]
        tabela_visivel(df_top, "Sem movimentacoes.")
    with col_d:
        st.subheader("🕐 Últimas 10 Saídas")
        df_ul = pd.read_sql_query("SELECT data_hora,entidade,material,quantidade,num_recibo FROM historico WHERE tipo='SAÍDA (CLIENTE)' ORDER BY id DESC LIMIT 10", conn)
    ...
    df_cliente_produto = pd.read_sql_query("""
        SELECT
            COALESCE(NULLIF(TRIM(h.entidade),''),'SEM CLIENTE') AS Cliente,
            COALESCE(NULLIF(TRIM(h.material),''),'SEM MATERIAL') AS Produto,
            COALESCE(TRIM(h.marca),'') AS Marca,
            COALESCE(TRIM(h.modelo),'') AS Modelo,
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if tipo_rel == "Estoque Atual Completo":
        ...
        if b_rc:
            df_rel = pd.read_sql_query("SELECT data_hora,entidade,material,marca,modelo,identificacao,quantidade,num_recibo FROM historico WHERE tipo='SAÍDA (CLIENTE)' AND entidade LIKE ? ORDER BY id DESC",
        ...
        if b_rf:
            df_rel = pd.read_sql_query("SELECT data_hora,entidade,material,marca,modelo,identificacao,quantidade FROM historico WHERE tipo='ENTRADA (FORNECEDOR)' AND entidade LIKE ? ORDER BY id DESC",
    ...
    try:
        with open(DB_PATH,"rb") as f: db = f.read()
        st.download_button("📥 Baixar Backup (.db)", db,
                           f"backup_{datetime.now().strftime('%d%m%Y_%H%M')}.db",
